...
const ProjectsBtn = () => {
  return (
    <div className="mx-auto xl:mx-0">
      <Link
        href={'/work'}
        className="relative w-[185px] h-[185px] flex items-center justify-center group hover:scale-110 transition-all duration-300"
      >
        {/* Black circle star - visible by default */}
        <div
          className="absolute inset-0 bg-cover bg-center bg-no-repeat transition-opacity duration-300 group-hover:opacity-0"
          style={{
            backgroundImage: 'url("/circle-star.svg")',
            filter:
              'brightness(0) saturate(100%) invert(0%) sepia(100%) saturate(0%) hue-rotate(0deg) brightness(0%) contrast(102%)',
          }}
        ></div>
        {/* Orange circle star - visible on hover, matches accent color */}
        ...
        <Image
          ...
          width={141}
          height={148}
          className="animate-spin-slow w-full h-full max-w-[141px] max-h-[148px] relative z-10"
          style={{
            filter: 'invert(1) grayscale(1) contrast(9999)',
            WebkitFilter: 'invert(1) grayscale(1) contrast(9999)',
          }}
        />
        <HiArrowRight className="absolute text-4xl text-dark group-hover:translate-x-2 group-hover:text-accent transition-all duration-300 z-20" />
      </Link>
    </div>
  );
};
